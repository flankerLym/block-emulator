...
from common import build_public_inputs_from_request, ensure_rvc_artifacts, load_vk


def main():
    req = json.load(sys.stdin)
    vk = load_vk(req.get("verifier_key_id", ""))
    _, _, vkey = ensure_rvc_artifacts(vk)
    ...
    if expected_digest != req.get("proof_digest"):
        ...
        return
    expected_public = [str(x) for x in build_public_inputs_from_request(req)]
    if [str(x) for x in payload.get("public", [])] != expected_public:
        json.dump({"ok": True, "valid": False}, sys.stdout)
        return
    with tempfile.TemporaryDirectory(prefix="zkscar_rvc_verify_") as td:
        td = Path(td)
