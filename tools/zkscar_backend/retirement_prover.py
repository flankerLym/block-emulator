...
def main():
    ...
    wasm, zkey, _ = ensure_retirement_artifacts(vk)
    if req.get("protocol_version") != vk.get("protocol_version") or req.get("circuit_version") != vk.get("circuit_version"):
        ...
        return
    circuit_input = build_retirement_circuit_input(req)
    with tempfile.TemporaryDirectory(prefix="zkscar_retirement_") as td:
        td = Path(td)
